...
def instance_already_running():
    lock_file_pointer = os.open(f"/tmp/fetch1PMplusAussen.lock", os.O_WRONLY | os.O_CREAT)
    try:
...
def insert_temp_aussen(id, ext_temp):
    insert_temperature(id, 'temp_aussen', ext_temp)


logging.basicConfig(filename='/home/dan/Documents/programming/IoT/dsnj-homie/fetch1PMplusAussen.log', format='%(asctime)s %(levelname)s: %(message)s', datefmt='%Y-%m-%d %H:%M:%S', level=logging.INFO)
...
try:
    ...
    cursor.execute('SELECT id, name, description FROM shelly_cfg WHERE description = \'shelly-1pm-plus-temp-outside\' ORDER BY id;')
...
# check if there's an addon and save then the aussen temp id=100 into the db
temp = get_temperature(id, 100)

if temp is not None:
    insert_temp_aussen(id, temp)
