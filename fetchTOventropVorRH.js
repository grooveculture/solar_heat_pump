...
async function insertData(){
    ...
    const sequelize = new Sequelize(process.env.DB_DB, process.env.DB_USER, process.env.DB_PASS, {
      ...
      const SpeicherTempRH = sequelize.define('oventrop_temp_rh', {
        temp: {
      ...
      .then(() => {
        ...
        SpeicherTempRH.bulkCreate([
          myobjs[0]
        ]).then(function () {
          return SpeicherTempRH.findAll();
        }).then(function (oventrop_temp_rh) {
          console.log(oventrop_temp_rh);
        });
      });
}