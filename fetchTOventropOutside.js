...
//    console.log(data);
    ...
    const newPage = await page.evaluate(() => {
        return document.getElementById('value_00_0010_7351_0100-016_2_0').innerHTML;  
    });

    console.log(newPage.substring(0, 3));
    temp = (newPage.substring(0, 3));
    insertData(temp);
    await browser.close()
...
async function insertData(){
    ...
    const sequelize = new Sequelize(process.env.DB_DB, process.env.DB_USER, process.env.DB_PASS, {
      ...
      const SpeicherTempOutside = sequelize.define('oventrop_temp_outside', {
        temp: {
      ...
      .then(() => {
        ...
        SpeicherTempOutside.bulkCreate([
          myobjs[0]
        ]).then(function () {
          return SpeicherTempOutside.findAll();
        }).then(function (oventrop_temp_outside) {
          console.log(oventrop_temp_outside);
        });
      });
}